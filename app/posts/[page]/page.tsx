...
export default async function PostsList({params,} : {params?: {page ? : number}}) {
  const currentPage = params?.page !== undefined ? params.page : 1
  // 현재 파라미터가 값이 없다면 1페이지가 되고 그게 아니라면 해당 페이지로 접속
  const perPage = 10
  const offset = (currentPage - 1) * perPage
  
  const [results] = await db.query<RowDataPacket[]>('SELECT * FROM brd.board order by date DESC limit ? offset ? ', [perPage, offset])
