        ...
        <>
            ...
            <div className="widget w-full overflow-hidden">
                <ul className="flex justify-between text-center py-2 bg-gray-50 text-xs sm:text-sm md:text-base">
                    ...
                    <li className="basis-1/3 md:basis-1/6">이메일</li>
                    <li className="basis-1/3 md:basis-1/6">이름</li>
                    <li className="basis-1/12">레벨</li>
                    <li className="hidden md:list-item basis-1/4">가입날짜</li>
                    <li className="basis-1/6">수정 / 삭제</li>
                </ul>
                {
                    data && data.map((e:memberInfo,i:number)=>{
                        ...
                        return(
                            <ul className="flex border-b last:border-0 justify-between text-center py-2 text-xs sm:text-sm md:text-base" key={i}>
                                ...
                                <li className="basis-1/3 md:basis-1/6">{e.email}</li>
                                <li className="basis-1/3 md:basis-1/6">{e.name}</li>
                                <li className="basis-1/12">{e.level}</li>
                                <li className="hidden md:list-item basis-1/4">{formatDate}</li>
                                <li className="basis-1/6"><Link href={`/admin/member/edit/${e.id}`}>수정</Link> / <MemberDelete name={e.name} id={e.id} /></li>
                            </ul>
                        )
                    })
                }
            ...
            <div className="flex justify-end mt-5">
                <Link href="/admin/member/add" className="bg-blue-500 text-white px-4 py-2 rounded-md shadow-md">회원추가</Link>
            </div>
        </>
    )
}