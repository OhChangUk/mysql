    ...
    if(req.method === 'POST'){
        ...
        try{
            ...
            if(!parentid || !userid || !username || !content){
                ...
                const [results] = await db.query<RowDataPacket[]>('insert into brd.comment (parentid, userid, username, content) values (?,?,?,?)', [parentid, userid, username, content])
                const [datas] = await db.query<RowDataPacket[]>('select * from brd.comment where parentid = ?', [parentid])
...
export const GET = async (
    ...
    if(req.method === 'GET'){
        try{
            ...
            const [results] = await db.query<RowDataPacket[]>('select * from brd.comment where parentid = ?', [parentid])
